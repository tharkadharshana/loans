...
export const getLoanAdvice = async (params: LoanParams): Promise<string> => {
  const { amount, interestRate, tenureMonths } = params;
  ...
  const prompt = `
    ...
    2. Highlight the trade-off: Suggest if slightly increasing the installment (shortening tenure) could save significant interest.
